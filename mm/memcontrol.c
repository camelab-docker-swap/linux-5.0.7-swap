...
static int mem_cgroup_swapfile_read(struct seq_file *sf, void *v)
{
	struct mem_cgroup *memcg = mem_cgroup_from_css(seq_css(sf));

	return swap_retrive_swap_device(&memcg->swap_type, sf);
}
static ssize_t mem_cgroup_swapfile_write(struct kernfs_open_file *of, 
						char *buf, size_t nbytes, loff_t off)
{
	struct mem_cgroup *memcg = mem_cgroup_from_css(of_css(of));

	return swap_store_swap_device(buf, &memcg->swap_type) ?: nbytes;
}

int mem_cgroup_get_swap_type(struct mem_cgroup *memcg)
{
	int type;
...
int mem_cgroup_get_page_swap_type(struct page *page)
{
	...
	rcu_read_lock();
	memcg = page->mem_cgroup;
	type = mem_cgroup_get_swap_type(memcg);
	rcu_read_unlock();
