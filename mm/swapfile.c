 ...
 */
EXPORT_SYMBOL_GPL(nr_public_swap_pages);
EXPORT_SYMBOL_GPL(nr_private_swap_pages);
			    ...
			    unsigned int nr_entries)
{
	...
	if (si->flags & SWP_PRIVATE)
		atomic_long_add(nr_entries, &nr_private_swap_pages);
	else
		atomic_long_add(nr_entries, &nr_public_swap_pages);
			       ...
			       swp_entry_t slots[])
{
	...
	if (si->flags & SWP_PRIVATE)
		...
		nearly_full = (atomic_long_read(&nr_public_swap_pages) * 2 < total_public_swap_pages);
...
int get_public_swap_pages(int n_goal, swp_entry_t swp_entries[], int entry_size)
{
	...
	avail_pgs = atomic_long_read(&nr_public_swap_pages) / size;
	if (avail_pgs <= 0)
		goto noswap;
	...
	atomic_long_sub(n_goal * size, &nr_public_swap_pages);
...
int get_private_swap_pages(int type, int n_goal, swp_entry_t swp_entries[], int entry_size)
{
	unsigned long size = swap_entry_size(entry_size);
	...
	avail_pgs = atomic_long_read(&nr_private_swap_pages) / size;
	if (avail_pgs <= 0)
		goto noswap;
	...
	atomic_long_sub(n_goal * size, &nr_private_swap_pages);
...
swp_entry_t get_swap_page_of_type(int type)
{
	...
	if (si->flags & SWP_WRITEOK) {
		atomic_long_t *counter = (si->flags & SWP_PRIVATE) ?
							&nr_private_swap_pages : &nr_public_swap_pages;
...
#ifdef CONFIG_MEMCG
int __attribute__((optimize(0)))swap_retrive_swap_device(const int *_type, struct seq_file *swap)
{
	struct swap_info_struct *si;
	struct file *file;
	int type;
...
int __attribute__((optimize(0)))swap_store_swap_device(const char *buf, int *_type)
{
